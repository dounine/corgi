...
<form target="uploadinfo" id="uploadForm" method="post" action="upload" enctype="multipart/form-data">
    <input type="hidden" id="uid" >
    <input type="file" id="fileDemo" multiple onchange="getFileSize(this.value)" name="file">
    <input name="fnames" id="fname" value="" >
    <input type="button" onclick="readProgress()" value="上传">
</form>
...
<script type="text/javascript">
    ...
    function readProgress(t){
        $.get("createUploadID",function(data){//创建文件上传事件ID
            if(data){
                $("#uploadForm").attr("action",data.data+"/upload")
                $("#uploadForm").submit();
                $("#uid").val(data.data);
                var index = 0;
                var progressInfo = setInterval(function () {
                    $.get(data.data+"/progress",function(data){
                        if(data){
