 ...
 */
public class JPAProviderFilterImpl implements ProviderFilter {

    private static final Logger LOGGER = LoggerFactory.getLogger(JPAProviderFilterImpl.class);
    private static final Map<String, TxObj> TX_OBJ_MAP = new HashMap<>();

    @Autowired
    protected JpaTransactionManager jtm;
    ...
    public void invokeAfter(Method method,Object result) {
        if (checkTxTransaction(method)) {
            ...
            TxObj txObj = new TxObj(jtm, ProviderJPATXContext.get(), LocalDateTime.now());
            new Thread(txObj);
